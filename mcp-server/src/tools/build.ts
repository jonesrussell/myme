...
import { runCommand, formatToolResult, type ToolResult } from "../utils/exec.js";
import { resolveAppPath } from "../utils/paths.js";
import path from "path";

const isWindows = process.platform === "win32";

export async function buildQt(): Promise<ToolResult> {
...
export async function cargoBuild(args: { package?: string }): Promise<ToolResult> {
  const { workspaceRoot } = getConfig();
...
export async function cargoTest(args: { package?: string }): Promise<ToolResult> {
  const { workspaceRoot } = getConfig();
...
export async function runApp(args: {
  ...
  const r = await runCommand(appPath, spawnArgs, { cwd: workspaceRoot, env: env as NodeJS.ProcessEnv });
  return r;
}
