...
const LOG = (msg: string) => console.error(`[myme-mcp] ${msg}`);
...
async function main() {
  ...
  server.registerTool(
    ...
    async (args: { mode?: string; env?: Record<string, string>; args?: string[] }) => {
      LOG("tool run_app");
      const r = await runApp({
        mode: args?.mode as "debug" | "release" | undefined,
        env: args?.env,
        args: args?.args,
      });
  ...
  server.registerPrompt(
    ...
    async (args: { providerName?: string }) => {
