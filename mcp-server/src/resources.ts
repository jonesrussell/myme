...
import { readResourceFile, readResourceFileSync } from "./utils/readFile.js";
...
async function getVersionContent(): Promise<string> {
  ...
  try {
    ...
    if (r.success && r.stdout) gitHash = r.stdout.trim();
  } catch {
    // ignore
  }
  return [
...
export function listResourceUris(): Array<{ uri: string; name: string; description?: string }> {
  return [
